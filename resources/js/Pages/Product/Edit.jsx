...
export default function Edit({ catgory, product }) {
    ...
    const { getRootProps, getInputProps } = useDropzone({
        ...
        onDrop: async acceptedFiles => {
            ...
            for (let i = 0; i < acceptedFiles.length; i++) {
                formData.append('product_images[]', acceptedFiles[i]);
            }
            ...
            try {
                ...
                for (let i = 0; i < response.data.length; i++) {
                    ...
                    setFiles(files => [...files, { url: response.data[i].url, id: files.length, image_id: response.data[i].image_id }]);
                }
            } catch (error) {
    ...
    const handleDelete = async (index, image_id) => {
        ...
        try {
            ...
            const newFiles = files.filter((item, i) => item.image_id !== image_id);
            setFiles(newFiles);
    ...
    useEffect(() => {
        ...
        setValue('product_detail', product.product_detail)
        setValue('product_status', product.product_status)
        setValue('cat_id', product.cat_id)

        product?.product_images.map((item, index) => {
            const url = `http://backoffice.amboss.cc/images/product/${product.id}/${item.image_names}`;
        ...
        <div style={thumb} key={file.id}>
            <div style={thumbInner}>
                ...
                <button
                    ...
                    className="absolute bottom-0 right-0 p-1 bg-red-500 rounded-full text-white text-xs"
                    style={{ zIndex: 1 }}
                >
                    Delete
                </button>
    ...
    const onSubmit = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.put(`/product/edit/`+product.id, data)
            }
        })
    }
            ...
            <div className="content-wrapper">
                ...
                <div className="container-lg bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="grid gap-6 mb-6 md:grid-cols-1  p-3">
                            <div>
